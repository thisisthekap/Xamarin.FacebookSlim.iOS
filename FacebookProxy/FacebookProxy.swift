...
@objc public class LoginResult: NSObject {
    ...
    public var declinedPermissions: Set<String> = []
}
    ...
    public func login(viewController: UIViewController, onCompleted: @escaping  (loginResultEnum, LoginResult?) -> Void) {
        ...
        LoginProxyManager.shared.loginManager.logIn(permissions: ["public_profile", "email"], from: viewController) { result, error in
            if let error = error {
                ...
                onCompleted(loginResultEnum.Erorr, nil)
...
@objc(CoreKitProxyManager)
public class CoreKitProxyManager : NSObject {
    
    @objc
    public static let shared = CoreKitProxyManager()
    private static let settings = FBSDKCoreKit.Settings()
    //private static let events = FBSDKCoreKit.AppEvents()
    ...
    public func enableLoggingBehavior(apploggingBehavior: ProxyLoggingBehavior) -> Void{
        ...
        CoreKitProxyManager.settings.enableLoggingBehavior(loggingBehavior)
    }
    
    @objc
    public func setAdvertiserTrackingEnabled(enabled : Bool) -> Void{
        CoreKitProxyManager.settings.isAdvertiserTrackingEnabled = enabled
    }
    
    @objc
    public func setAdvertiserIdCollectionEnabled(enabled : Bool) -> Void{
        CoreKitProxyManager.settings.isAdvertiserIDCollectionEnabled = enabled
    }
